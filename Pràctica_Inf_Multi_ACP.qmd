...
<<<<<<< HEAD
Hem de emplear una comparació de mitjanes, en un principi en aquestos casos ens demanen que totes les variables segueixes una normal multivariant on comparteixen la matriu de covarianzes. En el nostre cas el propi exercisi ja ens diu que aixo se satisfa. Es a dir totes son normal bivariant i tenen $\Sigma$ com a matriu de covariances, per tant podem garantir ses hipotesis.
Vist això, podrem realitzar la comparació de mitjanes, on la nostra hipotesi nula sera $H_0\colon \space \mu_T = \mu_R = \mu_I$ i la hipotesi alternativa $H_1 \colon \space \exists i,j \in \{T,I,R\} | \space\mu_i \neq \mu_j, i\neq j$ . Doncs si suposam que la hipotesi nul·la es certa tindrem que la nostra $B \sim W_2(\Sigma,3)$ $W \sim W_2(\Sigma,27)$ y $T \sim W_2(\Sigma, 29)$ on $B$ Y $W$ sin estocasticament independets i per tnat podemos garantir que seguira una distribucio de wilks. $\frac{\lvert W \rvert}{\lvert W+B\rvert} \sim\Lambda (2, 27, 29)$ i rebutjarem Ho si $\Lambda$ valor petit i significatiu.

### b. Escribe el estadístico razón de verosimilitudes

Com hem mencionat a l'apartat anterior volem fer un contrast de mitjanes. Sabem que a n'aquests casos l'estadístic de raó de versemblança ve donat per $-n\operatorname{log}(\Lambda)$ sent $\Lambda$ la lambda de Wilks aquesta distribució sent la hipòtesi nul·la certa ha de seguir una distribució 
$\chi^2$ amb $p(g-1)$ graus de llibertat. On p és el nombre variables i g el nombre de possibles vectors de mitjanes. En aquest problema p = 2 i g = 3.

=======
### b. Escribe el estadístico razón de verosimilitudes

>>>>>>> 75bb1933f3355ff53dd679476b3199387c04ca04
...
Notem que estadísticament tenim un n relativament gran, $n = n_1 + n_2 + n_3 = 10+8+12=30$. Sabem que $-n\operatorname{log}(\Lambda)$ segueix de manera asimptòtica una distribució $\chi^2$ amb $p(g-1)$ graus de llibertat. Sent $\Lambda$ la lambda de Wilks definida per:

$$
\Lambda = \frac{|\operatorname{W}|}{|\operatorname{T}|}
$$ amb W la matriu de covariància dins dels grups i T la matriu de covariància total. L'enunciat del problema ens proporciona quines són, en el nostre cas:
...
Calculem numèricament la lambda de Wilks:

```{r}
W <- matrix(c(520, 120, 120, 410), nrow = 2, ncol = 2, byrow = TRUE)
a = det(W)
T <- matrix(c(1340, 380, 380, 730), nrow = 2, ncol = 2, byrow = TRUE)
b = det(T)
lw = a/b
```

Després dels càlculs tenim que: $\Lambda$ és: `r round(lw,4)`. Comprovem finalment si tenim evidència suficient per rebutjar la hipòtesi nul·la:
...
est_contr = -n*log(lw)
p_valor = 1 - pchisq(est_contr,(g-1)*p)
```

Com tenim un p-valor 6 magnituts menor (p_valor = `r p_valor`) al nivell de significància establert tenim evidència suficient per rebutjar la hipòtesi nul·la.
...
A l'apartat anterior hem vist que tenim evidència estadística suficient per rebutjar la hipòtesi de què tots els vectors de mitjanes són iguals. Llavors, ha d'haver-hi almenys dues zones de la ciutat on la qualitat de l'aire sigui significativament diferent. A simple vista, pareix que la zona que més es diferencia de les altres és la zona residencial, ja que presenta nivells bastant menors de concentració tant de $\text{NO}_2,$ com de $\text{PM}_{2.5}$.

Seria interessant fer un contrast d'hipòtesi per recolzar de forma estadística que efectivament un dels vectors de mitjanes diferents és el de la zona residencial, però ens falten dades per poder-ho fer.

En conclusió, hem vist que té sentit rebutjar la hipòtesi nul·la fet que implica que els vectors de mitjanes són diferents. Això indica que hi ha zones de la ciutat que tenen millor qualitat d'aire, sent les zones residencials les que presenten menor concentració dels contaminants.
